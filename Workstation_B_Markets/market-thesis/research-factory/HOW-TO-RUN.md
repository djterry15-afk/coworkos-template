...
1. **Bring a seed.** One research question. Optionally: source pointers you already know, and a scope
   ...
   *Example seed (this template's sample):* "When financial stress hits, which market metrics move first,
   and which only confirm the move after it is already underway?"
...
6. **After the run.** The brief is yours to reason over → update the parent market-thesis `CONTEXT.md` (the verdict

...
4. **Extract (judgment):** pull **verbatim quotes + pointers** → `output/<slug>/signal-moments.md`. Never paraphrase; keep the snapshot reachable by `02_draft` for texture. **Tag authorship** on each pull — *the operator's own words* (usable as voice + texture) vs *retained AI output inside a source* (usable as substance only, **never** quoted as your voice — ROOT rule: raw assistant output is reasoning history, not synthesized voice). Authorship is a **separate axis from guard 6**: a source can be settled (guard-6 clean) yet AI-authored.

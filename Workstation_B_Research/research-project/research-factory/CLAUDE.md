...
- **the operator *concludes*** — reasons over the brief → the verdict lands in the parent research project `CONTEXT.md`.
...
- On completion (brief accepted into the parent research project `CONTEXT.md`): archive the run's working
  files → `_archive/<slug>/`.
